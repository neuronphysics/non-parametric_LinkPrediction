...
/**
 * Example: if we input count = 2, startK = 1, OriginalZn = 2002, combination = "", we will generate Zn copy
 * 2002, 2102, 2112, 2012. Change from the 2nd digit, and modify <count> digits in total, have all different combinations
 * @param count Input, combination length
 * @param startK Input, the position on Zn where we start to modify Zn copy
 * @param combination Input, temp storage of how we want to modify Zn copy
 * @param ZnOrigin Input, original Zn copy
 * @param records Output, store all the generated Zn copy
 */
void
generate_zn_combination(int count, int startK, unordered_map<string, gsl_matrix *> *records, const string &combination,
                        const gsl_matrix *ZnOrigin) {
    if (count == 0) {
...
/**
 * Example, K = 7, startK = 1, call this function will generate a new Zn with new Zn1, Zn2, Zn3, Zn4 entries.
 * These 4 entries are sampled simultaneously, but the result is identical to sample them sequentially
 * @param N Input, number of user in total
 * @param n Input, current sampling user index
 * @param K Input, current number of features
 * @param startK Input, the index of the feature we will start to sample from
 * @param D Input, number of attributes
 * @param nest Input, nest[i] define the number of 1s in the ith feature of Z matrix
 * @param s2Rho Input, variance of Rho
 * @param s2Y Input, variance of Y
 * @param C Input, define the attribute type of each attribute
 * @param R Input, for special attribute type only, define the number of choices of that attribute
 * @param Qnon Input, Q matrix generated by Z matrix without nth row
 * @param Enon Input, Eta matrix generated by Z matrix without nth row
 * @param Snon Input, S matrix generated by Z matrix without nth row
 * @param Znon Input, Z matrix without nth row
 * @param Rho Input, Rho matrix
 * @param Y Input, Y matrix
 * @param lambdanon Input, lambda matrix generated by Z matrix without nth row
 * @param Zn Input & Output, define the original nth row of Z matrix and will return the new nth row of Z matrix
 * @return the next unsampled index
 */
 ...
 * @param n Input, current sampling user index
 * @param K Input, current number of features
 * @param k Input, the feature index we are currently working on
 * @param given Input, either 0 or 1, will compute the probability that the kth entry of Zn is the <given> number
 * @param D Input, number of attributes
 * @param s2Rho Input, variance of Rho
 * @param s2Y Input, variance of Y
 * @param C Input, define the attribute type of each attribute
 * @param R Input, for special attribute type only, define the number of choices of that attribute
 * @param Zn Input, define the original nth row of Z matrix
 * @param Qnon Input, Q matrix generated by Z matrix without nth row
 * @param Enon Input, Eta matrix generated by Z matrix without nth row
 * @param Snon Input, S matrix generated by Z matrix without nth row
 * @param Znon Input, Z matrix without nth row
 ...
 * @param Y Input, Y matrix
 * @param lambdanon Input, lambda matrix generated by Z matrix without nth row
 * @param like Output, record the probability that the kth entry of Zn is the <given> number
 */
void compute_pseudo_likelihood_given_znk(int D,
    ...
    *like = init_likelihood_given_znk(D, K, n, s2Y, C, R, aux, ZnCopy, Y, lambdanon);
    LOG(OUTPUT_CONCURRENT, "-- like%d=%f\n", given, *like);
 ...
 * @param k Input, the feature index we are currently working on
 * @param D Input, number of attributes
 * @param nCount Input, the number of 1s in the kth feature of Z matrix
 * @param s2Rho Input, variance of Rho
 * @param s2Y Input, variance of Y
 * @param C Input, define the attribute type of each attribute
 * @param R Input, for special attribute type only, define the number of choices of that attribute
 ...
 * @param Y Input, Y matrix
 * @param lambdanon Input, lambda matrix generated by Z matrix without nth row
 * @param p Output, p[0] stores the Max(probability kth entry of Zn is 0, kth entry is 1),
 *              only the p[0] of last feature is meaningful, other p[0] are discarded
 */
void sample_znk(int N,
    ...
    if (nCount > 0) {
        ...
        LOG(OUTPUT_CONCURRENT, "lik0=%f , lik1=%f", lik0, lik1);
        double p0 = gsl_sf_log(N - nCount) + lik0;
        double p1 = gsl_sf_log(nCount) + lik1;
        double p1_n, p0_n;
        LOG(OUTPUT_CONCURRENT, "p1=%f, p0=%f", p1, p0);
...
/**
 * Step 1 of computing the probability that kth entry of Zn is the given number
 *
 * @param n Input, current sampling user index
 * @param K Input, current number of features
 * @param D Input, number of attributes
 * @param aux Input, Zn * Snon
 * @param s2Y Input, variance of Y
 * @param C Input, define the attribute type of each attribute
 * @param R Input, for special attribute type only, define the number of choices of that attribute
 * @param Zn Input, nth row of Z matrix with kth entry changed to given number
 * @param Y Input, Y matrix
 * @param lambdanon Input, lambda matrix generated by Z matrix without nth row
 * @return the probability that kth entry of Zn is the given number (not the final result yet)
 */
...
/**
 * Step 2 of computing the probability that kth entry of Zn is the given number
 *
 * @param N Input, total number of users
 * @param K Input, current number of features
 * @param r Input, current sampling user index, equivalent to n
 * @param zn Input, nth row of Z matrix with kth entry changed to given number
 * @param s2Rho Input, variance of Rho
 * @param Qnon Input, Q matrix generated by Z matrix without nth row
 * @param Znon Input, Z matrix without nth row
 * @param Enon Input, Eta matrix generated by Z matrix without nth row
 * @param Rho Input, Rho matrix
 * @param lik Input & Output, input the result from step 1 and output the final probability that the kth entry of zn is
 *              the given number
 */
