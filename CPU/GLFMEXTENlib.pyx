...
cdef extern from "../core/InferenceFunctionsNetwork.h":
...
cdef extern from "../core/InferenceFunctionsNetwork.h":
    int IBPsampler_func (double missing, gsl_matrix *X, char *C, char *NET, gsl_matrix *Z, gsl_matrix **B, gsl_vector **theta, double delta, gsl_matrix *H, gsl_matrix *A, int *R, double *f, double fa, double *mu, double *w, int maxR, int bias, int N, int D, int K, double alpha, double s2B, double *s2Y, double s2Rho, double s2H, double s2u, int maxK,int Nsim)
    ...
    #           s2Y: variance for pseudo-observations of attributes
    #           s2Rho: variance for pseudo-observations of adjacency matrix
    #           s2H: variance for homophily values
...
def infer(np.ndarray[double, ndim=2, mode="c"] Xin not None,\
        ...
        NETin not None, np.ndarray[double, ndim=2, mode="c"] Ain not None,\
        np.ndarray[double, ndim=1, mode="c"] Fin, double F, int bias=0, double s2u=1.0,\
        double s2B=1.0, double s2H=0.005, double alpha=1.0, int Nsim=100,\
    ...
    cdef int N, D, K
    cdef gsl_matrix_view Xview, Zview, Aview
    cdef gsl_matrix* X
    cdef gsl_matrix* A
    cdef gsl_matrix* Zm
    ...
    Aview = gsl_matrix_view_array(&Ain[0,0], N,N)
    A = &Aview.matrix

    cdef Pool mem = Pool()
    cdef gsl_matrix** B = <gsl_matrix**>mem.alloc(D, sizeof(gsl_matrix*));
    cdef gsl_vector** theta = <gsl_vector**>mem.alloc(D, sizeof(gsl_vector*));
    cdef double delta =0.; # initialize the free parameter of truncated Guassian
    cdef gsl_matrix *H = gsl_matrix_calloc(maxK,maxK); ###???
    ...
    if (Net=='w'):
        ...
        s2Rho=1
    cdef int Kest = IBPsampler_func(missing, X, C, Net, Z, B, theta,\
            H, A, <int*> R.data, &Fin[0], F, &mu[0], &w[0],\
    ...
    cdef np.ndarray[double, ndim=2] H_out = np.zeros((Kest,Kest))
...
#        print "s2Y_out loaded"
    ...
    return (Z_out,B_out,theta_out,H_out, mu, w, s2Y,s2Rho)
